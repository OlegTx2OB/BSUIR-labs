...
void Device::register_handle(HWND hWnd)
{
    ...
    HANDLE deviceHandle = CreateFile(reinterpret_cast<LPCSTR>(devicePath.c_str()), 0, FILE_SHARE_READ | FILE_SHARE_WRITE, NULL, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL);
...
Device::Device(HDEVINFO deviceList, SP_DEVINFO_DATA deviceInfo, HWND hWnd)
{
    ...
    this->name = std::wstring(reinterpret_cast<const wchar_t *>(buffer));
    ZeroMemory(buffer, sizeof(buffer));
    SetupDiGetDeviceRegistryProperty(deviceList, &deviceInfo, SPDRP_HARDWAREID, nullptr, (BYTE*)buffer, 1024, nullptr);
    this->HARDWARE_ID = std::wstring(reinterpret_cast<const wchar_t *>(buffer));
    ...
    if (hWnd != nullptr && this->ejectable) {
        ...
        devicePath = std::wstring(reinterpret_cast<const wchar_t *>(devInterfaceDetailData->DevicePath));
