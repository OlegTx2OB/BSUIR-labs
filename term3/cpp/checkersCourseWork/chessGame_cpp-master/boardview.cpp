...
void BoardView::initializeFiguresFields(QList<FigureClass*> pawns)
{
    for (int i = 0; i < pawns.length(); i++) {
        FigureClass *pawnModel = pawns[i];
        FigureField *pawn = new FigureField(pawnModel->position, pawnModel->imagePath, this);
...
void BoardView::moveActiveFigureToMousePosition(QPoint point, FigureClass *pawn) {
    int xPosition = point.x() - BoardField::defaultWidthHeight/2;
    int yPosition = point.y() - BoardField::defaultWidthHeight/2;
    FigureField *pawnField = getFigureAtBoardPosition(pawn->position);
...
void BoardView::placeActiveFigureAtBoardPosition(FigureClass *pawn, BoardPosition boardPosition) {
    FigureField *pawnField = getFigureAtBoardPosition(pawn->position);
...
void BoardView::removeFigureAtBoardPosition(BoardPosition boardPosition) {
    FigureField *pawnField = getFigureAtBoardPosition(boardPosition);
...
void BoardView::setFigureMoveCheckWarning(bool visible) {
    int opacity = visible ? 1 : 0;
    checkWarningTitleTextItem->setOpacity(opacity);
    checkWarningDescriptionTextItem->setOpacity(opacity);
}

void BoardView::promoteFigureAtBoardPosition(BoardPosition boardPosition) {
    FigureField *pawn = getFigureAtBoardPosition(boardPosition);
...
FigureField* BoardView::getFigureAtMousePosition(QPoint point) {
    for (int i = 0; i < pawns.length(); i++) {
        FigureField *pawn = pawns[i];
...
FigureField* BoardView::getFigureAtBoardPosition(BoardPosition boardPosition) {
    for (int i = 0; i < pawns.length(); i++) {
...
void BoardView::createFieldsColumn(int xPosition, int columnNumber)
{
    for (int rowNumber = 0; rowNumber < numberOfRowsColumns; rowNumber++)
    {
        ...
        if (columnNumber % 2 == 0)
        {
            if (rowNumber % 2 == 0) backgroundColor = QColor(196, 178, 140);
            else backgroundColor = QColor(157, 128, 101);
        }
        else
        {
            if (rowNumber % 2 == 0) backgroundColor = QColor(157, 128, 101);
            else backgroundColor = QColor(196, 178, 140);
        }
